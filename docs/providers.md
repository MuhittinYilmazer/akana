...
Akana registers five chat providers under a shared dispatch layer (`akana_server/orchestrator/llm_dispatch.py`). There is **no default provider**: with nothing configured, chat calls fail with a `No LLM provider configured` error (HTTP 503) rather than picking one for you.
...
> **Experimental:** Gemini, OpenAI and Ollama are wired through the same dispatch hub as Cursor and Claude, but with a different tool-delivery mechanism (native function-calling schemas rather than a mounted MCP server). They expose the same memory and vault tool set as the MCP providers (see "How dispatch works" for the mechanism). Their paths are covered by unit tests but see less end-to-end use than Cursor and Claude. Treat them as experimental. Tool parity is not the same as model capability (see the Ollama caveat below).
...
- For MCP-speaking providers (**Cursor**, **Claude**) it composes an `mcp_servers` payload delivered through the CLI/SDK. They see the built-in `akana_memory` and `akana_vault` MCP servers plus anything declared in `mcp_servers.yaml`.
- For the function-calling providers (**Gemini**, **OpenAI**, **Ollama**) it re-declares the same memory (`memory_search` / `save_memory` / `memory_forget`) and vault (all seven) tools as native schemas. The declarations are generated from the same schemas as the MCP tools, so the two surfaces stay identical. External MCP servers reach these providers through an in-process bridge (`mcp_bridge.py`) that surfaces them as `mcp__<server>__<tool>` functions. The tool loop caps at **five rounds per turn**.

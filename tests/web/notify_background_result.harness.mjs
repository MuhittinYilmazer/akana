 ...
 *   4. never fires for the user's OWN turn (they know it is running),
 *   5. permission is asked ONLY on a user gesture (browsers reject a bare request), once,
 *   6. clicking the notification focuses the window and opens that conversation,
 *   7. repeats for one chat REPLACE each other (tag) instead of stacking.
 *
...
// ── 1. silent while the user is watching it happen ──────────────────────────
{
  ...
  const fired = r.N.onTurnCompleted("c1", {}, { isCurrent: true });
...
// ── 2. fires when the user could NOT have seen it ───────────────────────────
{
  ...
  check("fires when the tab is hidden (even for the current chat)", () => {
    assert.equal(r.N.onTurnCompleted("c1", {}, { isCurrent: true }), true);
...
}
{
  ...
  check("fires for a result in ANOTHER conversation", () =>
    assert.equal(r.N.onTurnCompleted("c9", {}, { isCurrent: false }), true));
}
...
// ── 3. permission / opt-out gates ───────────────────────────────────────────
{
  ...
  check("never fires without permission", () => {
    assert.equal(r.N.onTurnCompleted("c1", {}, { isCurrent: false }), false);
    assert.equal(r.shown.length, 0);
  });
}
{
...
// ── 4. the user's own turn is not announced ─────────────────────────────────
{
  const r = boot({ hidden: true });
  check("a turn the user themselves sent is not announced", () =>
    assert.equal(r.N.onTurnCompleted("c1", { source: "user" }, { isCurrent: false }), false));
}

// ── 5. permission is gesture-bound and asked once ───────────────────────────
{
  const r = boot({ permission: "default", hidden: true });
  check("nothing is asked before a user gesture", () => assert.equal(r.requested(), 0));
  r.fireGesture();
  check("the first gesture asks for permission", () => assert.equal(r.requested(), 1));
...
// ── 6. click focuses the window and opens that conversation ─────────────────
{
  const r = boot({ hidden: true });
  r.N.onTurnCompleted("c9", {}, { isCurrent: false });
  r.shown[0].onclick();
...
// ── 7. repeats replace instead of stacking ──────────────────────────────────
{
  const r = boot({ hidden: true });
  r.N.onTurnCompleted("c1", {}, { isCurrent: false });
  r.N.onTurnCompleted("c1", {}, { isCurrent: false });
...
// ── setEnabled round-trip ───────────────────────────────────────────────────
{
  ...
  check("turning it back on restores it", () => {
    ...
    assert.equal(r.N.onTurnCompleted("c1", {}, { isCurrent: false }), true);
  });
}

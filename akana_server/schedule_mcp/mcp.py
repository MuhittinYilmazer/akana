"""ScheduleEngine MCP server — serves schedule tools via stdio JSON-RPC.

Structurally identical to :mod:`akana_server.vault_mcp.mcp` (each built-in MCP
server owns its own protocol loop; no shared base). Protocol: line-delimited
JSON-RPC 2.0; stdout is protocol-only, logs go to stderr.

The process is ``data_dir``-scoped, fed from ``AKANA_DATA_DIR``. Schedules are
plain JSON in ``<data_dir>/schedules.json`` (no crypto, no loopback REST) — the
child reads/writes the store directly, same pattern as the ``akana_memory`` /
``akana_vault`` children.

Run::

    AKANA_DATA_DIR=~/.akana python -m akana_server.schedule_mcp.mcp
"""
...
from akana_server.schedule_mcp.tools import ScheduleTools, schedule_schemas
...
SERVER_INFO = {"name": "akana-schedule", "version": "0.1.0"}
...
def mcp_tool_list() -> list[dict[str, Any]]:
    """Schedule schemas in MCP format (``inputSchema``, underscore-separated names)."""
...
class McpServer:
    ...
    def __init__(self, tools: ScheduleTools) -> None:
        self._tools = tools
...
def serve(tools: ScheduleTools, stdin: TextIO, stdout: TextIO) -> None:
...
def build_tools(data_dir: Path) -> ScheduleTools:
    """Setup seam that ``main()`` runs and tests drive directly.

    ``AKANA_CONVERSATION_ID`` (set per turn by the payload builder) carries the
    CALLER's conversation so a ``schedule_create`` from this child defaults to
    SAME-CHAT delivery — its fires are injected back into that conversation."""
    return ScheduleTools(
        data_dir,
        created_by="assistant",
        origin_conversation=os.environ.get("AKANA_CONVERSATION_ID", ""),
    )


def main() -> int:
    # WINDOWS CRITICAL: stdout/stderr default to the locale code page (cp1252/cp1254),
    # not UTF-8. The protocol writes JSON with ensure_ascii=False; a Turkish schedule
    # title (ı/ş/ğ…) would raise UnicodeEncodeError and kill the server mid-stream. MCP
    # stdio is UTF-8 by spec — force it (mirrors the memory + vault MCP servers).
    ...
    log.info("akana-schedule MCP serving on stdio (data_dir=%s)", data_dir)
